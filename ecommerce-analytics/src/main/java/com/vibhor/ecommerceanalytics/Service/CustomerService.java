...
@Service // Class where business logic is done
public class CustomerService {
    @Autowired //Dependency Injection where Repo is used
    private CustomerRepository customerRepository;
        public List<CustomerResponseDTO> getAllCustomers()
        {
            return  customerRepository.findAll().stream()
                    .map(customers -> CustomerMapper.entityToDto(customers)).
                    toList();
        }
            ...
            public Optional<CustomerResponseDTO> getCustomerbyId(Integer id)
        {
            return  customerRepository.findById(id).map(customers ->
                CustomerMapper.entityToDto(customers) );
        }
        public CustomerResponseDTO addCustomer(CustomerRequestDTO dto)
        {
            customers customer = CustomerMapper.DtoToEntity(dto); //DTO -> Entity
            customers savedCustomer = customerRepository.save(customer);
            CustomerResponseDTO responseDTO = CustomerMapper.entityToDto(savedCustomer);
             return  responseDTO;
        }
            public  CustomerResponseDTO UpdateCustomer(Integer id, CustomerRequestDTO dto)
            {
                 ...
                 if(existing.isPresent()) {
                     ...
                     CustomerResponseDTO UpdatedCustomer =  CustomerMapper.entityToDto(savedCustomer);
