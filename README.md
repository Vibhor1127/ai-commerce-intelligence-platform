...
![MySQL](https://img.shields.io/badge/MySQL-8.0_TiDB_Cloud-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-7.0_Upstash_Cache-DC382D?style=for-the-badge&logo=redis&logoColor=white)
...
**A Cloud-Native, High-Throughput E-Commerce Ecosystem & Executive Business Intelligence Platform.**  
Combines **Spring Boot 3.4 (Java 21)**, **Redis 7.0 In-Memory Caching**, **Spring AI (NVIDIA NIM Nemotron-3 550B)**, and a **Dual-Persona Glassmorphism & Cyberpunk React SPA**.

[🌐 Live Web Application](https://ai-commerce-intelligence-platform.vercel.app) • [📖 Interactive Swagger API Docs](https://ecommerce-backend-jnt4.onrender.com/swagger-ui/index.html) • [🐳 Docker Multi-Container](https://github.com/Vibhor1127/ai-commerce-intelligence-platform)
...
- [✨ Why ACI Stands Out](#-why-aci-stands-out)
...
1. **Dashboard Overload & Metric Blindness:** Business executives must navigate dozens of static charts to find answers to simple operational questions (e.g., *"Which products are losing margin due to delayed deliveries?"*).
...
```mermaid
graph TD
    Client["🌐 Web Browser / Executive Terminal"] -->|HTTPS / TLS 1.3| Vercel["☁️ Vercel Edge CDN<br/>React 18 + Vite SPA"]
    
    Vercel -->|REST API Calls / JWT Bearer| Render["⚡ Render.com Web Service<br/>Spring Boot 3.4 (Java 21) Docker"]
    
    subgraph Backend Engine ["Spring Boot Backend Engine"]
    ...
    ServiceLayer <-->|Sub-millisecond Read/Write Cache| Upstash[("⚡ Upstash Redis 7.0<br/>TLS Encrypted Distributed Cache")]
    ServiceLayer <-->|HikariCP Connection Pool / JDBC| TiDB[("🗄️ TiDB Cloud / MySQL 8.0<br/>14-Table Relational Schema")]
    ServiceLayer <-->|OpenAI-Compatible Chat Protocol| Nvidia["🧠 NVIDIA NIM AI Cloud<br/>Nemotron-3-Ultra-550B LLM"]
```
...
sequenceDiagram
    ...
    participant DB as TiDB MySQL Database
...
```mermaid
erDiagram
    APP_USERS ||--o| CUSTOMERS : "owns profile"
...
* Caches heavy aggregations (e.g., Monthly Revenue Run-Rate, Customer Lifetime Value, Top Products by Quantity).
